...
crawl xs ys coord_fun = map product (map nums cell_coords)
    where
      num_in_cell (x, y) = (grid !! y) !! x
      nums coords = map num_in_cell coords
      cell_coords = map cells [(x, y) | x <- xs, y <- ys]
      cells (x, y) = [(x, y), coord_fun (x, y), coord_fun (coord_fun (x, y)), coord_fun (coord_fun (coord_fun (x, y)))]

main :: IO ()
main = print $ result011
